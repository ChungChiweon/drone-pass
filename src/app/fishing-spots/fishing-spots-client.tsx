...
const fishingSpotTypes: { value: FishingSpotType; label: string }[] = [
  { value: "boat-fishing-point", label: "선상낚시포인트" },
  { value: "rock-fishing-point", label: "갯바위낚시포인트" }
];

function getFishingSpotTypeLabel(type: FishingSpotType) {
      ...
      <div className="space-y-5">
          ...
          <div className="relative p-6 sm:p-8">
            ...
            <div className="relative">
              ...
              <p className="mt-3 max-w-2xl text-sm font-semibold leading-7 text-sky-50 sm:text-base">
                해양수산부 공식 출조 포인트 데이터를 기반으로 선상낚시와 갯바위낚시 거점을 검색합니다. 실제 출조 가능 여부와 안전 통제는 현장 및 공식 안내를 반드시 확인하세요.
              </p>
              ...
              <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
                ...
                <div className="rounded-2xl bg-white/10 p-4">
                  <p className="text-2xl font-black">{getTypeCount("boat-fishing-point")}</p>
                ...
                <div className="rounded-2xl bg-white/10 p-4">
                  ...
                  <p className="mt-1 text-xs font-bold text-sky-100">갯바위</p>
                </div>
                <div className="rounded-2xl bg-white/10 p-4">
              ...
              >
                ...
                {fishingSpotTypes.map((item) => (
                  <option key={item.value} value={item.value}>
                    {item.label} ({getTypeCount(item.value)})
                  </option>
                ))}
              </select>
            </label>
        ...
        <section className="rounded-[2rem] border border-sky-100 bg-white p-5 shadow-sm sm:p-6">
          <div className="mb-5 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-xs font-bold text-slate-500">지도 좌표 확인 완료 {completeCoordinateCount.toLocaleString()}개 / 좌표 보강 필요 {(filteredSpots.length - completeCoordinateCount).toLocaleString()}개</p>
            </div>
            ...
            <div className="space-y-4">
              <div className="grid gap-3 lg:grid-cols-2">
                {pagedSpots.map((spot) => {
                  ...
                  const targetFish = spot.targetFish.split("|").filter(Boolean);
                        ...
                        <div className="mt-4 space-y-3 rounded-2xl bg-white p-4">
                          ...
                          <div className="rounded-2xl bg-amber-50 p-3">
                            ...
                            <p className="mt-1 text-xs font-semibold leading-5 text-slate-600">
                              실제 출조 가능 여부, 출입 통제, 선박 운항, 기상 위험은 현장 및 공식 안내를 확인하세요.
                            </p>
                          </div>
                        </div>
                      ) : null}
        ...
        <section className="rounded-[2rem] border border-amber-100 bg-amber-50 p-5 shadow-sm">
          ...
          <p className="mt-2 text-sm font-semibold leading-6 text-amber-900/80">
            이 페이지는 공식 포인트 원본을 Blue Marina 표준 필드로 정리한 1차 데이터입니다. 실제 낚시 가능 여부, 출입 통제, 선박 운항, 기상 위험은 출조 전 공식 안내와 현장 상황을 반드시 확인하세요.
          </p>
